...
const SignUp = () => {
    ...
    const isCapital = (labelValue) => /[A-Z]/.test(labelValue);
    const isLower = (labelValue) => /[a-z]/.test(labelValue);
    const isSpecial = (labelValue) => /[ `!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/.test(labelValue);
    const isNumber = (labelValue) => /\d/.test(labelValue);
    const isSpaces = (labelValue) => /\s/.test(labelValue);
    ...
    const sendData = async () => {
        ...
        if (!(privacyPolicy && terms)) {
            alert('Terms and conditions', 'Please accept Terms & Conditions/Privacy Policy');
            return;
        }
        ...
        try {
            ...
            const response = await axios.post('https://chronedo.webjerky.com/api/register', data);
            console.log('response--------', response)
            if (response.data.success) {
                router.push('/verifyEmail');
            } else {
                alert('Registration Failed', response.data.message || 'Something went wrong');
            }
        } catch (error) {
            console.error(error);
            ...
            <div className={styles.innerContainer}>
                ...
                <div className={styles.conditionsContainer}>
                    ...
                    {(special || number) ? (
                        ...
                        <div className={styles.condition}>
                            <div className={styles.uncheckedIcon}></div>
                            <p className={styles.errorText}>A digit {"[123] / special character [! $ @]"}</p>
                        </div>
                    )}
                ...
                <div className={styles.checkboxContainer}>
                    <label className={styles.checkboxLabel}>
                        <div className={styles.customCheckbox} onClick={() => setPrivacyPolicy(!privacyPolicy)}>
                            {privacyPolicy && <Image src="/assets/icons/on.png" alt="Checked" width={20} height={20} />}
                        </div>
                        I accept the privacy policy.
                    </label>
                    <label className={styles.checkboxLabel}>
                        <div className={styles.customCheckbox} onClick={() => setTerms(!terms)}>
                            {terms && <Image src="/assets/icons/on.png" alt="Checked" width={20} height={20} />}
                        </div>
                ...
                <button className={styles.socialButton} onClick={() => console.log("LinkedIn login")}>
                    <div className={styles.socialRectangle}>
                        in
                    </div>
                    Continue with LinkedIn
                </button>

                <button className={styles.socialButton} onClick={() => console.log("Google login")}>
                    <div className={styles.socialRectangle}>
                        G
                    </div>
